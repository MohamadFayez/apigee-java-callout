...
public class KeyUtil {
  ...
  public static PrivateKey decodePrivateKey(String privateKeyPemString, String password)
    ...
    try {
      ...
      if (o instanceof PrivateKeyInfo) {
        ...
        // org.bouncycastle.jcajce.provider.asymmetric.ec.BCECPrivateKey or RSAPrivateKey

        return (PrivateKey) converter.getPrivateKey((PrivateKeyInfo) o);
      }

      if (o instanceof PKCS8EncryptedPrivateKeyInfo) {
        System.out.printf("PKCS8EncryptedPrivateKeyInfo");
      ...
      if (o instanceof PEMEncryptedKeyPair) {
        System.out.printf("PEMEncryptedKeyPair");
  ...
  public static byte[] decodeSecretKey(String key, String keyEncoding) {
    if (keyEncoding == null) return key.getBytes(StandardCharsets.UTF_8);
    keyEncoding = keyEncoding.toLowerCase();
    if (keyEncoding.equals("utf-8") || keyEncoding.equals("utf8"))
      return key.getBytes(StandardCharsets.UTF_8);
    if (keyEncoding.equals("base16")) return Base16.decode(key);
    if (keyEncoding.equals("base64")) return Base64.getDecoder().decode(key);
    if (keyEncoding.equals("base64url")) return Base64.getUrlDecoder().decode(key);
    throw new IllegalStateException("unknown key-encoding");
  }
}
